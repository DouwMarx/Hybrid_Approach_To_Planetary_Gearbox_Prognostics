...
#             "That":13}
#
#save_obj(dictionary,"Dict_test")
#
#d = load_obj("Dict_test")
#print(d["This"])
...
def import_ring(bdf_file):
    ...
    py_send("*cbody_param_table prot " + Rotation_table)  # Rotation table 
...
def import_planet(bdf_file):
    ...
    # Create a geometrical contact body on the outside of the ring gear
    py_send("*add_nodes 0 " + str(planet_carrier_pcr) + " 0") #Add the reference nodes for the contact body
    py_send("*add_nodes 0 " + str(planet_carrier_pcr + 5) + " 0") #Add the reference nodes for the contact body
    ...
    py_send("*set_curve_type circle_cr") #Curve type to circle
    planet_centre = "0 " + str(planet_carrier_pcr) + " 0 "
    ...
    planet_inside_bot = planet_carrier_pcr - planet_axle_radius
    py_send("*select_clear")
...
def results():
    #Select the two nodes that will be used to calculate the deflection angle
    s = selection("Planet_Axle")
    nlist = s.find_axle_nodes()
    
    for node in nlist:
        make_history_plot(node)
    return 
...
def main():
    ...
    model_name = 'work_on_output_file'
