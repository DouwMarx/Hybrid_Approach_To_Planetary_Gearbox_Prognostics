...
def import_ring(bdf_file):
    ...
    py_send("*contact_value prot 0.05")    #rotation total magnitude
...
def import_planet(bdf_file,move_y,move_rz):
    ...
    planet_radius = str(29.32/2)#
    py_send("*add_curves " + planet_centre + planet_radius) # Draw the circle
    ...
    py_send("*apply_dof y *apply_dof_value y")
    py_send("*apply_dof x *apply_dof_value x")
    py_send("*apply_dof mz *apply_dof_value mz 5e12 moment_3") #Set the moment magnitude here
...
def main():
    ...
    import_planet(planet_bdf,-1.9,-1.7)
