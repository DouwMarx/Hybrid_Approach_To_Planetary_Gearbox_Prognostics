...
def cost_m1_init_cond_meas1_state(theta):
    ...
    lmm = s.LMM_sys(M, C, K, f, X0, Xd0, t)
    sol = lmm.solve_de("RK")
    return np.log(np.linalg.norm(sol[:, -3] - measured[:, -3]))
...
optimum_m1_init = opt.differential_evolution(cost_m1_init_cond, bnds, polish=True, maxiter=0)
...
startpoint = np.array([1, 0, 0, 0, 0, 0, 0])
optimum_m1_init_1state = opt.differential_evolution(cost_m1_init_cond_meas1_state, bnds, polish=True, popsize=1000)#, maxiter=0, popsize=100)
...
sol = optiminum_plot(optimum_m1_init_1state['x'])
plt.plot(sol[:,-3])